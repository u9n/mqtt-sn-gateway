...
from asyncio_mqtt import Client as MqttClient, MqttCodeError
...
class MQTTSNGatewayServer:
    ...
    broker_clients: List[MqttClient] = attr.ib(factory=list)
        ...
        if client:
            ...
            if topic:
                try:
                    ...
                    # it is an error that should reset the connection.
                    # Should a timeout error reset the connection?
                    raise
        ...
        while should_reconnect:
            try:
                async with AsyncExitStack() as stack:
                    ...
                    broker_connection_count = 100
                    LOG.info(
                        f"Setting up {broker_connection_count} connections to MQTT broker at localhost:1883 "
                    )

                    clients = [
                        await stack.enter_async_context(MqttClient("localhost"))
                        for _ in range(0, broker_connection_count)
                    ]
